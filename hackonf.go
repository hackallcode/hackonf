...
func getFilename(paths []string) (string, error) {
...
func load(file string, output interface{}) (err error) {
...
func Load(paths []string, output interface{}) error {
	file, err := getFilename(paths)
	if err != nil {
		return err
	}
	err = load(file, output)
	if err != nil {
		return err
	}
	parse(output)
